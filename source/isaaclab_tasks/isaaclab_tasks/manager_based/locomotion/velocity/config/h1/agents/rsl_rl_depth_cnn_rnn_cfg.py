...
"""Configuration for RSL-RL depth CNN + RNN/LSTM policy for H1."""
...
@configclass
class H1RoughDepthCNNRecurrentRunnerCfg(RslRlOnPolicyRunnerCfg):
    ...
    policy = RslRlActorCriticDepthCNNRecurrentCfg(
        # Standard depth CNN parameters (same as non-RNN)
        init_noise_std=1.0,
        actor_hidden_dims=[512, 256, 128],
        critic_hidden_dims=[512, 256, 128],
        activation="elu",
        # H1 proprio: base_lin_vel(3) + base_ang_vel(3) + projected_gravity(3) +
        #             velocity_commands(3) + joint_pos(19) + joint_vel(19) + actions(19) = 69
        num_actor_obs_prop=69,
        # Depth image size matching legged-loco: 53x30 = 1590 pixels
        obs_depth_shape=(53, 30),
        # RNN-specific parameters
        rnn_type="lstm",           # or "gru"
        rnn_input_size=256,        # Match hidden_dims[-1]
        rnn_hidden_size=256,       # LSTM hidden state size
        rnn_num_layers=1,          # Number of stacked LSTM layers
        num_critic_obs_prop=69,    # H1 critic proprioception observations
    )
